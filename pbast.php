      ...
      <div class="col-sm-8 mb-4">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="col-lg-12" id="div_bast">
                    <label>Tanggal BAST</label>
                    <div class="input-group" id="input_bast">
                    <input type="text" class="form-control date" id="tgl_bast" placeholder="00/00/0000">
                      
                      <span class="input-group-addon bast">
                        <input type="button" id="btn_add_bast" value="+" style="height: 33px; width: 25px;" onclick="addBAST()">
                      </span>
                    </div><!-- /input-group -->
          ...
          <div id="rs-table"></div>
          <div id="bast-table"></div>

          <script>
          var i_bast = 1;
          
          function addBAST() {
            const node_input = document.getElementById("input_bast");
            const clone_input = node_input.cloneNode(true);
            clone_input.id = "clone_bast"+i_bast++;
            
            clone_input.querySelector("#btn_add_bast").setAttribute("value", "-");
            clone_input.querySelector("#btn_add_bast").setAttribute("onclick", "removeBAST("+clone_input.id+")");
            document.getElementById("div_bast").appendChild(clone_input);
          ...
          function getTglBAST() {
            const bast_tgl = document.querySelectorAll('[id="tgl_bast"]');
            
            
            //const bast_qty = document.querySelectorAll('[id="qty_bast"]');
            //const qty = [...bast_qty].map(input => input.value) *qty:qty[index]*;
                
                var arr = [...bast_tgl].map(function(val, index){
                    return {no:index, tanggal:val.value};
                })
                
                
                return arr.map((o)=> JSON.stringify(o));
                
                
          }
          function removeBAST(key) {
              document.getElementById(key.id).remove();
              //document.querySelectorAll(`[id*="clone_bast"]`).forEach(e => e.remove());
          }
          function resetBAST() {
              //document.getElementById(key.id).remove();
              document.querySelectorAll(`[id*="clone_bast"]`).forEach(e => e.remove());
              i_bast = 1;
                  ...
                  input.addEventListener('change', function() {
                    ...
                    document.getElementById('rs-table').innerHTML =
                      ...
                      '<label style="color: red; text-align: right; width: 100%; margin-top:10px;">Total : Rp. '+ Math.round((jumlah * 100) / 100).toLocaleString() +'</label>'
                      ...
                      console.log(JSON.stringify(dataTable.data));
...
<br>
  ...
  <script type="text/javascript">
    ...
    $.ajaxSetup({ cache: false });
      ...
      $(".buat").click(function(){
         if(validasi()){
            ...
            var tgl_bast = getTglBAST();
            var dataPost = {...dform, tgl_bast, order};
    
            $.ajax({
                   url: 'data/laporan/create.php',
                   type: 'post',
                   data: {data:dataPost},
                   complete:function(data){
                      ...
                      setTimeout(function() {
                       ...
                       dataTable.data = [];
                      $this.button('reset');
                      OpenWindow(resp["idlap"], resp["bast"]);
                      console.log(resp);
                      }, 500);
    ...
    function pilihpj(){
      ...
      if (pilipj==='Direktur') {
        $("#nama_pj").val("Susi Nurhandayani, S.Pd");
      } else if (pilipj==='Komisaris') {
        $("#nama_pj").val("Ahmad Sukron Ma'mun");
      } else {
        $("#nama_pj").val("");
      }
    ...
    function OpenWindow(id, qbast) { 
      
      var html = '<a href="data/laporan/print.php?spj=kwi&id=' + id + '"  target="_blank"><button style="margin: 2px; width: 70%;"><b style="font-size: 17px;">Kwitansi</b></button></a><br>' +
                  '<a href="data/laporan/print.php?spj=po&id=' + id + '"  target="_blank"><button style="margin: 2px; width: 70%;"><b style="font-size: 17px;">Purchase Order</b></button></a><br> ' +
                  '<a href="data/laporan/print.php?spj=inv&id=' + id + '"  target="_blank"><button style="margin: 2px; width: 70%;"><b style="font-size: 17px;"> Invoice</button></b></a><br>' +
                  '<a href="data/laporan/print.php?spj=spk&id=' + id + '"  target="_blank"><button style="margin: 2px; width: 70%;"><b style="font-size: 17px;"> SPK</button></b></a><br>' +
                 '';
      // looping from i = 1 to 5
        for (let i = 1; i <= qbast; i++) {
            html = html + '<a href="data/laporan/print.php?spj=bast&bast='+ i +'&id=' + id + '"  target="_blank"><button style="margin: 2px; width: 70%;"><b style="font-size: 17px;"> BAST '+ i +'</button></b></a><br>';
        }
      let timerInterval
    Swal.fire({
      title: 'Loading ...!',
  ...
  function validasi() {
		...
		if (npsn != "") {
			...
			alert('Pilih data pelanggan dulu !');
		}
	}
